...
class DslUserStoriesTest < ActionDispatch::IntegrationTest
  ...
  def test_two_people_buying
    dave = regular_user
      mike = regular_user
    dave.buys_a @ruby_book
      mike.buys_a @ruby_book
    dave.has_a_cart_containing @ruby_book
    assert_not_equal dave.session[:cart_id], mike.session[:cart_id]
    dave.checks_out DAVES_DETAILS
      mike.has_a_cart_containing @ruby_book
    check_for_order DAVES_DETAILS, @ruby_book
      #mike.checks_out MIKES_DETAILS
      #check_for_order MIKES_DETAILS, @ruby_book
  end
  ...
  def regular_user
    open_session do |user|
      ...
      def user.checks_out(details)
        ...
        assert_response :success
        assert_template "new"
        old_cart = Cart.find(session[:cart_id])
        ...
        cart = Cart.find(session[:cart_id])
        assert_not_equal old_cart, cart
